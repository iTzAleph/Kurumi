...
module.exports = {
  name: 'dance',
  description: 'Baila solo o con quien quieras nwn',
  aliases: ['bailar'],
  ...
  execute(message, client, args) {
    ...
    const user= client.users.cache.find(e =>  e.username.startsWith(args[0])) ||client.users.resolve(args[0]) || message.mentions.users.first()
    ...
    if(!args[0]) { 
      ...
      var frase = baile2 = [
        `> **${message.author.username}** y **${user.username}** estan bailando`,
        `> **${message.author.username}** a empezado a bailar con **${user.username}**`,
        `> **${message.author.username}** baila para **${user.username}**`,
        `> **${user.username}**, parece que **${message.author.username}** quiere bailar contigo`,
      ]}
